...
if menu == "🔍 Análisis de empresa":
    ...
    if st.button("✨ Analizar empresa"):
        if ticker_input.strip():
            with st.spinner("Obteniendo datos financieros y generando análisis..."):
                try:
                    ...
                    if not info or info.get("regularMarketPrice") is None:
                        ...
                        precio = alt.Chart(hist).mark_line(color=bright_pink).encode(x="Date:T", y="Close:Q")
                        promedio = alt.Chart(hist).mark_line(strokeDash=[4, 4], color=pale_gold).encode(x="Date:T", y="MA20:Q")
